...
<?php 
    echo $this->load->view('header'); $ctlr = "fin"; 
    $sum_data = $this->fin_model->sum_data_by_oid($data2['oid'], $data2['id'], 'selesai');
    $sisa = $data2['jml'] - $sum_data['selesai'];
?>
...
<div class="control-group row-fluid">
    <label class="control-label offset3" for="jumlah">Selesai <span class="required">*</span></label>
    <div class="controls">
        <input class="offset1" id="jumlah" type="text" name="selesai" maxlength="11" value="<?php echo (!empty(set_value('selesai'))) ? set_value('selesai') : ((!empty($data['selesai'])) ? $data['selesai'] : $sisa); ?>"  />
    </div>
</div>
...
<div class="control-group row-fluid">
    ...
    <div class="controls">
        <div class="input-append date offset1" id="dpto" data-date="<?php echo date('Y-m-d'); ?>" data-date-format="yyyy-mm-dd">
            <input type="text" id="tgl" name="tgl" value="<?php echo (!empty(set_value('tgl'))) ? set_value('tgl') : ( (!empty($data['tanggal'])) ? $data['tanggal'] : date('Y-m-d') ) ?>"  />
