...
<div class="table-responsive">
   <table class="table table-hover">
       <thead>
      <tr>
          <td class="success">Orderid: </td><td class="success"><div class="text-right"><?php echo $data2['id']; ?></div></td>
          <td class="warning">Item: </td><td class="warning"><div class="text-right"><?php echo $data2['itemjahitan']. ' ' .$data2['bahanboneka']. ' ' .$data2['warnaboneka']; ?></div></td>
          <td class="info">Jumlah: </td><td class="info"><div class="text-right"><?php echo $data3['jml']; ?></div></td>
      </tr>
      </thead>
   </table>
</div></div></div>
...
if($mode == 'insert') {
    echo form_open('prtl/insert', $attributes, $hidden);
} else {
    echo form_open('prtl/insert', $attributes, $hidden);
}
?>
...
<div class="control-group row-fluid">
    ...
    <div class="controls">
        <!--<input class="offset1" id="status" type="text" name="status" maxlength="11" value="<?php echo (!empty(set_value('status'))) ? set_value('status') : ((!empty($data['status'])) ? $data['status'] : ''); ?>"  />-->
    <?php $options = array(
                        '0'  => 'Pengerjaan',
                        '1' => 'Selesai',
                    );
            $attr = "id='status' class='offset1'";
            $defVal = (!empty(set_value('status'))) ? set_value('status') : ((!empty($data['status'])) ? $data['status'] : '');
    ?>
        <?php echo form_dropdown('status', $options, $defVal, $attr); ?>
    </div>    
</div>
...
<div class="control-group row-fluid">
    ...
    <div class="controls">
        <div class="input-append date offset1" id="dpto" data-date="<?php echo date('Y-m-d'); ?>" data-date-format="yyyy-mm-dd">
            <input type="text" id="tgl" name="tgl" placeholder="Di isi setelah selesai." value="<?php echo set_value('tgl'); echo (!empty($data['selesai'])) ? $data['selesai'] : ''; ?>"  />
