...
class Kapasitas extends CI_Controller {
    ...
    function insert() {
        ...
        if ($this->authentication->is_signed_in()) {
            ...
            if (!$this->authorization->is_permitted(['update_kapasitas', 'create_kapasitas'])) {
        ...
        $this->form_validation->set_rules('activity', 'Bagian', 'required|max_length[20]');
        $this->form_validation->set_rules('kapasitas', 'Kapasitas', 'required|is_numeric|max_length[11]');
        ...
        if ($this->form_validation->run() == FALSE) { // validation hasn't been passed
            ...
            if (empty($dataid)) {
                if (!$this->authorization->is_permitted(['create_kapasitas'])) {
                    redirect('home/blocked');
                }
                // run insert model to write data to db
                if ($this->kapasitas_model->SaveForm($form_data) == TRUE) { // the information has therefore been successfully saved in the db
                    $this->session->set_flashdata('success', $form_data['activity'] . ' sukses ditambah');
                    $form_data['id'] = $this->activitylog->get_id('cfg_kapasitas');
                    $this->activitylog->save_log($data['account'], $this->ctlr, 'create', $form_data);
                } else {
                    $this->session->set_flashdata('success', $form_data['activity'] . ' gagal ditambah');
                }
            } else {
                if (!$this->authorization->is_permitted(['update_kapasitas'])) {
                    redirect('home/blocked');
                }
                $this->kapasitas_model->update_data($dataid, $form_data);
                $this->session->set_flashdata('success', $form_data['activity'] . " berhasil diupdate");
                $form_data['id'] = $dataid;
    ...
    function index($slug = NULL) {
        ...
        if ($this->authentication->is_signed_in()) {
            $data['account'] = $this->account_model->get_by_id($this->session->userdata('account_id'));
            if (!$this->authorization->is_permitted(['retrieve_kapasitas'])) {
                redirect('home/blocked');
            }
        }
        $this->load->helper('form');
        ...
        $rows = $this->kapasitas_model->list_data($slug, NULL, $searchtext, $orderby, $asc);
...
//        $data['loginas'] = $this->loginas;
        ...
        $data['records'] = $this->kapasitas_model->list_data($slug, $limit, $searchtext, $orderby, $asc);
    ...
    public function del($slug = NULL) {
        if ($this->authentication->is_signed_in()) {
            ...
            if (!$this->authorization->is_permitted(['delete_kapasitas'])) {
                redirect('home/blocked');
            }
        }
        $this->load->helper('form');
        $dataid = $this->input->post("dataid");
        $sqldata = $this->kapasitas_model->get_single_data($dataid);
        $result = $this->kapasitas_model->sql_rm_data($dataid);
        if ($result) {
            $data['success'] = "Data " . $sqldata['activity'] . " sukses dihapus";
            $this->activitylog->save_log($this->account, $this->ctlr, 'delete', $sqldata);
        } else {
            $data['success'] = "Data " . $sqldata['activity'] . " gagal dihapus";
        }
    ...
    public function edit() {
        ...
        $data['data'] = $this->kapasitas_model->get_single_data($dataid);
        $data['mode'] = 'edit';
        $data['success'] = '';
    ...
    public function json() {
        $rows = $this->kapasitas_model->json_data();
